...
function runHook(cjsPath: string, stdinPayload: string, env?: Record<string, string>): Promise<{ code: number; stdout: string }> {
  return new Promise((resolve, reject) => {
    const proc = spawn("node", [cjsPath], {
...
test("settings template commands resolve via package bins (no absolute paths)", () => {
  ...
  for (const c of cmds) {
    assert.ok(c in pkg.bin, "command must be a package bin: " + c);
...
test("single-source: regenerating configs is byte-identical (no drift)", () => {
  const r = spawnSync(process.execPath, [join(PLUGIN_ROOT, "scripts", "gen-claude-configs.mjs")], { encoding: "utf8" });
  assert.equal(r.status, 0, "generator must run clean: " + r.stderr.slice(0, 200));
  // Re-read and compare against a second regen — generator is deterministic,
  // so the on-disk files must equal freshly generated bytes.
  ...
  try {
    ...
    const r2 = spawnSync(process.execPath, [join(PLUGIN_ROOT, "scripts", "gen-claude-configs.mjs")], { encoding: "utf8" });
    assert.equal(r2.status, 0);
    const after = files.map(f => readFileSync(join(PLUGIN_ROOT, f), "utf8"));
...
testAsync("session-start: envelope additionalContext carries the routing card", async () => {
