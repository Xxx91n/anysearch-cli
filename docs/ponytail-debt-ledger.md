...
apps/plugin/src/hooks/adapters/codebuddy.ts:49 | CodeBuddy SessionStart has no .mdc rules-file fallback — routing card relies solely on stdout-to-context injection (ADR-0011 D6 forbids writing host rule files). | Add fallback channel if CodeBuddy ships a rules-file mechanism
apps/mcp/src/tools/ans-chat.tool.ts:71 | PiAgentRuntime was constructed with models: undefined (fixed — now passes session.models); adjacent LLM session cache key now includes a sha256 digest of ANS_LLM_API_KEY (F-A4). | Revisit cache-key scope if per-request model override lands
