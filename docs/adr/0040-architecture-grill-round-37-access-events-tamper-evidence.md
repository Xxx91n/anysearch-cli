...
**D2 (component set A') — Three components plus schema_version.** (1) `prev_hash` hash chain (SHA-256) over canonical events; (2) independent zero-dependency `scripts/verify-access-events.mjs` wired into ship-gate step 1; (3) the instrumentation `catch {}` becomes counted telemetry `eventWriteFailures` in the Observational zone (ADR-0039 report-as-contract; never gated — Goodhart). Plus `schema_version` + `event_type` columns from day one (Sigilbase step 1: immutable logs must version). Explicitly rejected with registered triggers: Ed25519 signatures (trigger: multi-user / service deployment — signatures only defend against the operator, who is the user), Merkle checkpoints (trigger: scale makes O(n) re-verification impractical), external anchoring / RFC 3161 (evaluate only for compliance delivery), idempotency keys (trigger: writes gain retries/concurrency — better-sqlite3 single-threaded writes have none), forward-secure truncation defense (local truncation risk accepted; git commits act only as manual checkpoints — by declaration, no mechanism binds the chain head or genesis anchor to git; see ADR-0041 D2). Chain guarantees start at ingestion (honest limits, Sigilbase).
...
6. Golden vectors byte-match RFC 8785 appendix cross-checks (adapted form: appendix vectors contain REAL/Unicode values outside the INTEGER/TEXT whitelist, so the cross-check is executed as the section 3.2.2/3.2.3 key-order equivalence assertion + literal byte-string vectors + empty-segment digest = sha256(''); see Implementation Notes).
...
6. r105 amendment (ADR-0041 D1): ship-gate step 1 gains a verification-object ladder — ANS_DB_PATH explicitly set but missing = misconfiguration fail; default DB exists = verify-what-you-consume; neither = gate-built track (scripts/chain-gate-fixture.ts via tsx, real SqliteSessionStore write path, deterministic seeds, .ship-gate/chain-gate.db). Gate passes only on exit 0 AND verdict "PASSED" double-check; ledger/report record object=consumed|gate-built. The D5 fixture prohibition stands: it bars fixtures masquerading as production verification objects; the gate-built track verifies the write-path contract of this very gate run (SLSA verify-what-you-build).
7. r105 amendment (ADR-0041 D2): RFC 8785 cross-check executed in adapted form (key-order equivalence + literal byte strings + empty digest = sha256('')) because appendix vectors exceed the INTEGER/TEXT whitelist; Acceptance 6 annotated accordingly.
8. r105 amendment (ADR-0041 D2): the git-anchor wording in D2 was claim without mechanism — softened to manual checkpoints only; threat model (non-adversarial operator) unchanged.
