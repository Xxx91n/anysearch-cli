...
**Status: 0.0.2 release candidate** — publish pending (manual first release; see [docs/publishing.md](docs/publishing.md)). Once live: `npm i -g @anysearch-cli/cli`.
...
- **Page-level precision is asserted but currently thin** — of the 12
  live-scoped `eval-looks.json` golden entries, 10 are quarantined for
  provider-side URL-shape drift (exact `mustHitUrl` paths vs returned
  `/zh/` locale, `/10.x` version, dated `/specification/<date>/` variants —
  ledger `packages/store/eval-quarantine.json`, TTL 2026-10-14, shrink-only
  ratchet). What stays asserted live: host-level coverage (all 12 entries'
  mustHitHosts, incl. the quarantined set's — host evidence is 12/12) and the
  online abstain leg (docs-g0014). Page-level exact-URL hard assertions are
  0/12 while the drift ruling is pending — do not read page precision off this
...
The complete numbered record lives in `docs/adr/` — ADR-0001 through ADR-0064.
