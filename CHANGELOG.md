...
- `packages/store/test/governed-json-canonical.test.mjs`（19 断言）+ `closeout-coverage.test.mjs`（40 断言，含审计返工 F-2/F-4 及二批 residual 回归用例）——known-bad fixture 层常态化红绿双证，随 `turbo test` 行使。
