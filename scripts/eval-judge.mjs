...
// ADR-0029 D3/D4: report carries rubricHash + judgeVersion (sixtuple snapshot with eval-calibrate.mjs);
// module is importable (judgeOne/RUBRIC) so the calibration script reuses the same judge client.
//
...
export const RUBRIC = "You are a strict retrieval-relevance judge. Answer with a single character: 1 if the returned snippet is relevant to the query, 0 otherwise.";
export const JUDGE_RUBRIC_HASH = createHash("sha1").update(RUBRIC).digest("hex").slice(0, 12);
export const judgeVersion = (model, baseUrl) => {
  try { return model + " @ " + new URL(baseUrl).host; } catch { return model + " @ " + baseUrl; }
};

export async function judgeOne(prompt, { baseUrl, model, apiKey }) {
...
async function main() {
  ...
  const out = {
    schema: "anysearch/judge-report@2",
    generatedAt: new Date().toISOString(),
    model,
    baseUrl,
    rubricHash: JUDGE_RUBRIC_HASH, // ADR-0029 D3/D4: snapshot component
    judgeVersion: judgeVersion(model, baseUrl),
    note: "nightly/manual report channel (ADR-0027 D2). Not a gate. Enable only after kappa >= 0.6 calibration (scripts/eval-calibrate.mjs).",
...
if (invoked && import.meta.url === pathToFileURL(invoked).href) {
  main().then(() => process.exit(0), (e) => { console.error("eval-judge: internal error: " + String(e && e.stack || e)); process.exit(2); });
}
