    ...
    const agent = new Agent({
      ...
      shouldStopAfterTurn: async (ctx: any) => {
        ...
        if (typeof lastTool === "string" && lastTool.includes("search")) lastSearchTurn = 1;
            ...
            // D1: Google SCA paradigm — gate outputs "what's missing" (named gap), not just boolean.
            // D5: bounded reround — default 1, max 3 via domain TOML.
            ...
            if (lastSearchResult && maxRerounds > 0) {
              try {
                ...
                if (envelopeJson) {
                  ...
                  if (suff && suff.verdict !== "correct") {
                    ...
                    if (streamFn && model) {
                      ...
                      if (gapQuery.length > 3) {
                        // D5: bounded re-search loop.
                        for (let round = 0; round < maxRerounds; round++) {
                          const reroundEnvelope = await this.opts.retriever.search({ query: gapQuery, mode: "fast" });
