...
async function main() {
  // Shape: 41 cases (ADR-0028: +15 D3/D4 slices; ADR-0031: +4 entity group; ADR-0033: +2 semantic cases), 11 groups, every op has a stage.
  assert(GOLDEN_CASES.length === 41, "41 golden cases (got " + GOLDEN_CASES.length + ")");
  const groups = new Set(GOLDEN_CASES.map((c) => c.group));
  assert(groups.size === 11, "11 groups (got " + groups.size + ")");
