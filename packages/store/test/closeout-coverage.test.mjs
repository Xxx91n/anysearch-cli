...
// --- rework-2 residuals -------------------------------------------------------
// F-5⑥: the parser is decoupled from the generator's prose wording — a
// different preamble sentence still parses; a non-table line in the block is
// skipped (it cannot carry a registration row); a malformed `|` row still reds.
const mini2 = mini.replace("The complete numbered record lives in `docs/adr/` — ADR-0001 through ADR-0002.", "Totally different preamble wording");
assert(parseRegisteredRounds(mini2).problems.length === 0 && parseRegisteredRounds(mini2).rounds.has(76), "preamble wording decoupled from parser (F-5⑥)");
{
  const r = parseRegisteredRounds(mini.replace("| [0002](0002-architecture-grill-round-76-y.md) | Grill Round 76 — Y |", "a stray prose line"));
  assert(r.rounds.size === 0 && r.problems.length === 0, "non-table line inside block is skipped (empty set surfaces at the (c) assertion, not here)");
}
// A mid-sentence round mention in a non-round ADR title must NOT register the
...
try {
  ...
  fs.mkdirSync(path.join(tmp, "grill-round-x", "handoffs"), { recursive: true });
