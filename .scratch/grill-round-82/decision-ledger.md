...
- **revised 2026-09-25（事实修正非改向）**：T0 tarball 实测 SessionStartSource 枚举=`startup|resume|clear|compact`——§1 票文 `'fresh'` 为 grill 期占位词，执行时 guard 应写 `source!=="startup"`（resume/clear/compact 不重复注路由卡语义不变）。双锚判据与裁决面不受影响；修正此前已三处披露（evidence/t0-watch-2026-09-24.md §5、issues/02、ADR-0083 D3），此处补账本字面通道（审计 O-1）。
- **状态**：current
