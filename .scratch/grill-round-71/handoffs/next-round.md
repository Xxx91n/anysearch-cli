...
生成依据：`.scratch/grill-round-71/decision-ledger.md`（唯一数据源，4 条 current：D-001/D-002/D-003/D-004）。
调研存档：`.scratch/grill-round-71/q2-atomcode.md`、`.scratch/grill-round-71/q3-atomcode.md`。
锐评原文：`.codex-tmp/锐评.txt`（第六轮）。
...
标的：`scripts/ship-gate.mjs`、`.github/workflows/release.yml`、`AGENTS.md`、`docs/adr/0071-architecture-grill-round-70-verification-timing-hardening.md`、`.scratch/grill-round-70/handoffs/round-70-closeout.md`。
...
3. **ship-gate fail-closed 新腿**（step-1 家族，与 SHA/URL/parity leg 并列）：可配置扫面清单（docs/**/*.md+.scratch 全 markdown 目录清单，新文档类型显式登记）；机器本地路径正则（^[A-Za-z]:[\\/]、/Users/、/home/、AppData、/tmp/，防 https: 误伤）；带完整治理字段的声明标记豁免。 <!-- machine-local: build-host temp path cited as evidence @ 2026-09-19 -->
4. **历史机械扫**：`D:\Aworker\anysearch-cli\` → repo-relative 无损变换（.scratch 入库文档全量+docs/adr 命中处）。 <!-- machine-local: repo location on the authoring host @ 2026-09-19 -->
5. **Temp transcript 入库**：`C:\Users\Administrator\AppData\Local\Temp\r70-audit\` 审计 transcript 复制入 `.scratch/grill-round-70/evidence/`（默认提交；仅确认无保留价值才改走哈希+销毁声明）。 <!-- machine-local: build-host temp path cited as evidence @ 2026-09-19 -->
6. **ADR-0071 四处+R70 closeout** 路径随票修。
7. **先红后绿证据对**：lint 腿先抓现存违例（红 transcript）→清扫+声明后转绿（绿 transcript），双件存档 `.scratch/grill-round-71/evidence/`。
...
标的：`apps/cli/src/commands/doctor.ts`、`scripts/install-smoke.mjs`、`README.md`（+zh-CN parity）、各发布包 `package.json`（devEngines 剥离面）。

顺序（三件套按序，commit 粒度可拆）：
1. **Spike 双臂**（先取证后动手）：npm 全局+pnpm 全局各一臂 clean install `@anysearch-cli/cli@0.0.5`+`@anysearch-cli/embedding@0.0.5`——四断言：(a) `ans doctor` 报 vector arm present；(b) `ans memory backfill-vectors` 对存量 memory 生效；(c) 卸载 embedding 后 Jaccard 降级无回归；(d) 首用下载失败报错质量（代理/离线可操作）。transcript 全量存档 `.scratch/grill-round-71/evidence/`。
...
1. **ADR-0072**（`docs/adr/0072-architecture-grill-round-71-*.md`）：主题+D-001~D-004 决策条目+Closure evidence 四段回填。
