...
const srv = http.createServer((req, res) => {
  let body = ''; req.on('data', c => body += c); req.on('end', () => {
    appendFileSync(LOG, 'ANS_STUB|' + req.method + ' ' + req.url + ' auth=' + (req.headers.authorization ?? 'none') + ' sess=' + (req.headers['x-anysearch-session-id'] ?? 'none') + ' tp=' + (req.headers['traceparent'] ?? 'none') + ' ts=' + (req.headers['tracestate'] ?? 'none') + '\n' + body + '\n');
    if (req.url === '/policy') {
